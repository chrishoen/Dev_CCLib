...
void TestNode::attachBeforeFirstChild (TestNode* aObjectNode)
{
...
void TestNode::attachAfterLastChild  (TestNode* aObjectNode)
{
   // If this node doesn't have any children
   // Attach the object node as the only child of this node
   if (this->mFirstChildNode == 0)
...
TestNode* TestNode::detachLastChild()
{
   // Pointer to the last child node of this subject parent node, this is
   // the node that is to be be detached.
   TestNode* tNodeToDetach = this->mLastChildNode;
...
void TestNode::attachBefore (TestNode* aObjectNode)
{
