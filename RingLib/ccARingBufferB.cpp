...
void ARingBufferBState::initialize(int aNumElements, size_t aElementSize, int aReadGap)
{
...
ARingBufferBWriter::ARingBufferBWriter()
{
   mRB = 0;
   mNumElements = 0;
   mElementSize = 0;
...
void ARingBufferBWriter::initialize(ARingBufferBState* aARingBufferBState, void* aElementArrayMemory)
{
   resetVars();
   mRB = aARingBufferBState;
...
void ARingBufferBWriter::resetVars()
{
   resetTest();
}
...
// of elements.
void* ARingBufferBWriter::elementAt(long long aIndex)
{
   aIndex %= mNumElements;
...
long long ARingBufferBWriter::getNextWriteIndex()
{
   // Return the index of the next element to write to.
...
void ARingBufferBWriter::doWrite(void* aElement)
{
...
void ARingBufferBWriter::doWriteArray(void* aElementSourceArray, int aNumElements)
{
...
void* ARingBufferBWriter::startWrite()
{
   // Get the index of the next element to write to.
   long long tWriteIndex = mRB->mNextWriteIndex.load(std::memory_order_relaxed);
...
void ARingBufferBWriter::finishWrite()
{
   if (mTestFlag)
   {
      // Get the index of the next element to write to.
...
ARingBufferBReader::ARingBufferBReader()
{
   mRB = 0;
   mNumElements = 0;
   mElementSize = 0;
...
void ARingBufferBReader::initialize(ARingBufferBState* aARingBufferBState, void* aElementArrayMemory)
{
   resetVars();
   mRB = aARingBufferBState;
   mNumElements = mRB->mNumElements;
...
void ARingBufferBReader::resetVars()
{
   mFirstFlag = true;
   mRestartAtMax = true;
   mLastReadIndex = 0;
...
void* ARingBufferBReader::elementAt(long long aIndex)
{
   aIndex %= mNumElements;
   return (void*)((char*)mElementArrayMemory + mElementSize * aIndex);
}
...
int ARingBufferBReader::available()
{
...
void ARingBufferBReader::doRestartAtMax()
{
   mFirstFlag = true;
   mRestartAtMax = true;
}
...
void ARingBufferBReader::doRestartAtMin()
{
   mFirstFlag = true;
   mRestartAtMax = false;
}
...
bool ARingBufferBReader::doRead(void* aElement)
{
   // Do this first.
   mNotReadyFlag = false;
   mOverwriteFlag = false;
...
void ARingBufferBReader::doUndoLastRead()
{
...
int ARingBufferBReader::doReadArray(void* aElementDestinArray, int aNumElements)
{
   int tCount = 0;
