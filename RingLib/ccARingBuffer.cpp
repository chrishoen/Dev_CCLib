...
void ARingBufferState::initialize(int aNumElements, size_t aElementSize, int aReadGap)
{
...
ARingBufferWriter::ARingBufferWriter()
{
   mRB = 0;
   mNumElements = 0;
   mElementSize = 0;
...
void ARingBufferWriter::initialize(ARingBufferState* aARingBufferState, void* aElementArrayMemory)
{
   resetVars();
   mRB = aARingBufferState;
...
void ARingBufferWriter::resetVars()
{
   resetTest();
}
...
// of elements.
void* ARingBufferWriter::elementAt(long long aIndex)
{
   aIndex %= mNumElements;
...
long long ARingBufferWriter::getNextWriteIndex()
{
   // Return the index of the next element to write to.
...
void ARingBufferWriter::doWrite(void* aElement)
{
...
void ARingBufferWriter::doWriteArray(void* aElementSourceArray, int aNumElements)
{
...
void* ARingBufferWriter::startWrite()
{
   // Get the index of the next element to write to.
   long long tWriteIndex = mRB->mNextWriteIndex.load(std::memory_order_relaxed);
...
void ARingBufferWriter::finishWrite()
{
   if (mTestFlag)
   {
      // Get the index of the next element to write to.
...
ARingBufferReader::ARingBufferReader()
{
   mRB = 0;
   mNumElements = 0;
   mElementSize = 0;
...
void ARingBufferReader::initialize(ARingBufferState* aARingBufferState, void* aElementArrayMemory)
{
   resetVars();
   mRB = aARingBufferState;
   mNumElements = mRB->mNumElements;
...
void ARingBufferReader::resetVars()
{
   mFirstFlag = true;
   mRestartAtMax = true;
   mLastReadIndex = 0;
...
void* ARingBufferReader::elementAt(long long aIndex)
{
   aIndex %= mNumElements;
   return (void*)((char*)mElementArrayMemory + mElementSize * aIndex);
}
...
int ARingBufferReader::available()
{
...
void ARingBufferReader::doRestartAtMax()
{
   mFirstFlag = true;
   mRestartAtMax = true;
}
...
void ARingBufferReader::doRestartAtMin()
{
   mFirstFlag = true;
   mRestartAtMax = false;
}
...
bool ARingBufferReader::doRead(void* aElement)
{
   // Do this first.
   mNotReadyFlag = false;
   mOverwriteFlag = false;
...
void ARingBufferReader::doUndoLastRead()
{
...
int ARingBufferReader::doReadArray(void* aElementDestinArray, int aNumElements)
{
   int tCount = 0;
